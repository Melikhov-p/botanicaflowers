...
# эти настройки.
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'botanicaflowers.settings')
# Создаем объект(экземпляр класса) celery и даем ему имя
app = Celery('botanicaflowers')
# Загружаем config с настройками для объекта celery.
# Т.е. импортируем настройки из django файла settings
# namespace='CELERY' - в данном случае говорит о том, что применяться будут только
# те настройки из файла settings.py которые начинаются с ключевого слова CELERY
app.config_from_object('django.conf:settings', namespace='CELERY')
# В нашем приложении мы будем создавать файлы tasks.py в которых будут находиться
